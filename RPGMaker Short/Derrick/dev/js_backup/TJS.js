...
const TJS_ITEM_TREASURE_TYPE_PRIMARY_NONE = 1;
const TJS_ITEM_TREASURE_TYPE_PRIMARY_COMMON = 2;
const TJS_ITEM_TREASURE_TYPE_PRIMARY_UNCOMMON = 3;
const TJS_ITEM_TREASURE_TYPE_PRIMARY_RARE = 4;
const TJS_ITEM_TREASURE_TYPE_SECONDARY_COMMON = 5;
const TJS_ITEM_TREASURE_TYPE_SECONDARY_UNCOMMON = 6;
const TJS_ITEM_TREASURE_TYPE_SECONDARY_RARE = 7;

const TJS_ITEM_SPECIAL_BROKEN = 1;
const TJS_ITEM_SPECIAL_NORMAL = 2;
const TJS_ITEM_SPECIAL_ENCHANTED = 3;
const TJS_ITEM_SPECIAL_FIRE = 10;
const TJS_ITEM_SPECIAL_ICE = 11;
const TJS_ITEM_SPECIAL_LIGHTNING = 12;
const TJS_ITEM_SPECIAL_METAL = 13;
const TJS_ITEM_SPECIAL_ACID = 14;
const TJS_ITEM_SPECIAL_WATER = 30;
const TJS_ITEM_SPECIAL_EARTH = 31;
const TJS_ITEM_SPECIAL_WIND = 32;
const TJS_ITEM_SPECIAL_DARKNESS = 35;
const TJS_ITEM_SPECIAL_IMMORTALITY = 36;
const TJS_ITEM_SPECIAL_MULTI_ELEMENTAL = 40;
const TJS_ITEM_SPECIAL_NATURE = 43;
const TJS_ITEM_SPECIAL_RAINBOW = 47;
const TJS_ITEM_SPECIAL_HASTED = 50;
const TJS_ITEM_SPECIAL_WEAPON_RUNE = 55;
const TJS_ITEM_SPECIAL_ARMOR_RUNE = 56;
const TJS_ITEM_SPECIAL_BLEEDING = 100;
const TJS_ITEM_SPECIAL_STUNNING = 110;
const TJS_ITEM_SPECIAL_ANTIMAGICAL = 120;
const TJS_ITEM_SPECIAL_POWER = 200;
const TJS_ITEM_SPECIAL_HEALING_HP = 1000;
const TJS_ITEM_SPECIAL_HEALING_MP = 1010;
const TJS_ITEM_SPECIAL_HEALING_TP = 1020;
const TJS_ITEM_SPECIAL_HEALING_DEATH = 1100;
const TJS_ITEM_SPECIAL_HEALING_OTHER = 1200;
const TJS_ITEM_SPECIAL_CURATIVE_SPECIFIC = 1500;
const TJS_ITEM_SPECIAL_CURATIVE_OMNI = 1550;
const TJS_ITEM_SPECIAL_ABILITY_BOOST = 1600;
const TJS_ITEM_SPECIAL_ABILITY_GROW = 1700;
const TJS_ITEM_SPECIAL_POWER_UP_GENERIC = 1800;
const TJS_ITEM_SPECIAL_FOOD = 1950;
const TJS_ITEM_SPECIAL_MONSTER_DROP = 2000;
const TJS_ITEM_SPECIAL_TREASURE = 2500;
const TJS_ITEM_SPECIAL_ORB = 3000;
const TJS_ITEM_SPECIAL_ORB_SHARDS = 3100;
const TJS_ITEM_SPECIAL_CLASS_BOOK = 20000;
const TJS_ITEM_SPECIAL_RUNE = 22000;
const TJS_ITEM_SPECIAL_GEAR_MAGICAL_RING_MINOR = 60000;
const TJS_ITEM_SPECIAL_GEAR_MAGICAL_RING_MAJOR = 60010;
const TJS_ITEM_SPECIAL_GEAR_MAGICAL_RING_ULTRA = 60020;
const TJS_ITEM_SPECIAL_GEAR_MAGICAL_RING_LEVELED = 60050;

var tjs_color_tp = "rgba(64, 255, 64, 1)";
var tjs_color_mp = "rgba(255, 64, 64, 1)";
...
TJS.prototype.GetInfo1 = function(item) {
    ...
    if ( item.wtypeId ) {
        if ( item.weapon_type == 1 ) {
            text = "\
(Broken Primary Weapon) Your weapon is broken.  Broken weapons cannot be fixed, and need to be de-equipped and replaced.";
        }
...
into following turns in combat, but instead do all of their damage right away, like most other swords, and mundane weaponry.  Longswords can be equipped by Swordmaster and Cleric class characters.";
        }
...
TJS.prototype.GetInfo2 = function(item) {
    ...
    if ( item.wtypeId ) {
        if ( item.weapon_type == 1 ) {
            ...
            if ( item.special == TJS_ITEM_SPECIAL_NORMAL ) text = "\
...
skills will eventually break after skills it grants are used enough times, so using a special sword attack that equipping a longsword grants will eventually wear out the sword.  Helpful hint: Keep spares.\\LB\
    ...
    Note that some items will last a long time, while other items will break after their skills are used only a few times.  (Magic wands, for example, are notorious for being very perishable and breaking rather quickly.)";
            ...
            if ( item.special == TJS_ITEM_SPECIAL_NORMAL ) text = "";
            if ( item.special == TJS_ITEM_SPECIAL_WIND ) text = "";
            if ( item.special == TJS_ITEM_SPECIAL_EARTH ) text = "";
            if ( item.special == TJS_ITEM_SPECIAL_ANTIMAGICAL ) text = "";
            if ( item.special == TJS_ITEM_SPECIAL_POWER ) text = "";
        }
        else if ( item.weapon_type == 13 ) {
            if ( item.special == TJS_ITEM_SPECIAL_NORMAL ) text = "\
...
very fiery.";
            else if ( item.special == TJS_ITEM_SPECIAL_FIRE ) text = "\
Fire Grenades explode into shards of material combined with heat and flames that shred, melt, and burn enemy targets. They do a combination of physical and fire damage.";
            else if ( item.special == TJS_ITEM_SPECIAL_ICE ) text = "";
            else if ( item.special == TJS_ITEM_SPECIAL_LIGHTNING ) text = "";
            else if ( item.special == TJS_ITEM_SPECIAL_ACID ) text = "\
...
do about the same comparable total damage spread out over time, give or take.";
            else if ( item.special == TJS_ITEM_SPECIAL_STUNNING ) text = "\
Stun grenades debilitate enemies by lowering certain ability scores for a couple of turns, and can stun targets for about 1 to 3 turns so they cannot act.  They do no damage.";
            else if ( item.special == TJS_ITEM_SPECIAL_ANTIMAGICAL ) text = "";
        }
        else if ( item.weapon_type == 14 ) {
            if ( item.special == TJS_ITEM_SPECIAL_NORMAL ) text = "";
            if ( item.special == TJS_ITEM_SPECIAL_WIND ) text = "";
...
TJS.prototype.CalcItemPrice = function(item) {
    ...
    /*
    if (item.meta) {
        if (item.meta.tjsFoodPrice) {
            price = TJS_auto.CalcFoodPrice( Number(item.meta.tjsFoodPrice) );
        } else {
            var item_level = -1;
            if ( item.meta.tjsItemLevel ) item_level = Number(item.meta.tjsItemLevel);
            if ( item.meta.tjsWeaponLevel ) item_level = Number(item.meta.tjsWeaponLevel);
            if ( item.meta.tjsLevel ) item_level = Number(item.meta.tjsLevel);

            if ( item_level > 0 ) {
                if ( (item.meta.tjsLeveledByChapterPrice) || (item.meta.tjsWeapon) || (item.meta.tjsGear) || (item.meta.tjsArmor) || (item.meta.tjsBook) ) {
                    price = TJS_auto.CalcLevelByChapterPrice( price, item_level );
                }
            }
        }
    }
...
TJS.prototype.CalcLevelByChapterPrice = function(base_price, item_level) {
    ...
    const factor1 = Math.pow( item_level, 3.0 );
    const factor2 = TJS_auto.LerpSkew( 1.0, tjs_max_chapters, chapter / tjs_max_chapters, -0.325 );
...
TJS.prototype.GetInfoItemExpanded = function(item, actor, text_width, text_command, text_columns=2, text_column_width=64) {
    ...
    */
    ...
    if ( text_command === "actor" ) {
        ...
        /*
        data_text += this.GetTextInfoActorTraitsSelected( $dataActors[actor.id].traits, "param", text_column_width, text_columns );
        data_text += this.GetTextInfoActorTraitsSelected( $dataActors[actor.id].traits, "xparam", text_column_width, text_columns );
        data_text += this.GetTextInfoActorTraitsSelected( $dataActors[actor.id].traits, "sparam", text_column_width, text_columns );
        data_text += this.GetTextInfoActorTraitsSelected( $dataActors[actor.id].traits, "attack", text_column_width, text_columns );
        data_text += this.GetTextInfoActorTraitsSelected( $dataActors[actor.id].traits, "debuff", text_column_width, text_columns );
        data_text += this.GetTextInfoActorTraitsSelected( $dataActors[actor.id].traits, "state immune", text_column_width, text_columns );
        data_text += this.GetTextInfoActorTraitsSelected( $dataActors[actor.id].traits, "state resist", text_column_width, text_columns );
        ...
        data_text += "\\LB";
        data_text += this.GetTextInfoActorSkills( $dataActors[actor.id].traits, text_column_width, text_columns );
        */
    ...
    if ( text_command === "enemy" ) {
        ...
        /*
        data_text += actor.name + ", a level " + actor.level + " combatant.\\LB";
        data_text += "Experience Points for defeat: " + (actor.exp).toLocaleString() + " EXP.\\LB";
        data_text += TextManager.currencyUnit + " gained for defeat: " + (actor.gold).toLocaleString() + " " + TextManager.currencyUnit + ".\\LB";
        data_text += "\\LB";

        data_text += "Natural ability tendencies regardless of class, level, or equipped gear:\\LB";
        data_text += "Initial / Maximum Tactical Points: " + Math.round(actor.initial_tp).toLocaleString() + " / " +
            Math.round(actor.custom_traits.max_tactical_points_add * actor.custom_traits.max_tactical_points_multiply).toLocaleString() + ".\\LB";
        data_text += this.GetTextInfoParameters( item, text_column_width, text_columns );
        data_text += this.GetTextInfoActorTraitsSelected( $dataEnemies[actor.id].traits, "param", text_column_width, text_columns );
        data_text += this.GetTextInfoActorTraitsSelected( $dataEnemies[actor.id].traits, "xparam", text_column_width, text_columns );
        data_text += this.GetTextInfoActorTraitsSelected( $dataEnemies[actor.id].traits, "sparam", text_column_width, text_columns );
        data_text += this.GetTextInfoActorTraitsSelected( $dataEnemies[actor.id].traits, "attack", text_column_width, text_columns );
        data_text += this.GetTextInfoActorTraitsSelected( $dataEnemies[actor.id].traits, "debuff", text_column_width, text_columns );
        data_text += this.GetTextInfoActorTraitsSelected( $dataEnemies[actor.id].traits, "state immune", text_column_width, text_columns );
        data_text += this.GetTextInfoActorTraitsSelected( $dataEnemies[actor.id].traits, "state resist", text_column_width, text_columns );
    ...
    // Skill.
        ...
        if ( text_command === "skill" ) {
            ...
            /*
            if ( skill_Tp_gain > 0 ) {
                data_text += "Tactical Point Gain: " + skill_Tp_gain + "\\LB";
            }
            else {
                data_text += "Tactical Point Cost: " + skill_Tp_cost + " (" + skill_Tp_cost_adjusted + ")\\LB";
            }
            */
    ...
    // State
        ...
        /*
            ...
            if ( item.multi_level_state_averaged > 0 )
                data_text += this.GetTextInfoTraitsGeneral( item.traits, text_column_width, text_columns );
            else
                data_text += this.GetTextInfoTraits( item.traits, text_column_width, text_columns );
        }
        */
...
TJS.prototype.DisplayTextEnemyInfo = function(actor_in, char_count_width) {
    ...
    /*
    var level_multiplier = (level / tjs_max_monster_level).clamp( 0.00000, 1.00000 );
    level_multiplier = 1.00000 - level_multiplier;
    level_multiplier = Math.pow( level_multiplier, 2.5 );
    level_multiplier = 1.00000 - level_multiplier;
    */
...
TJS.prototype.GetTextInfoItemLevel = function(item) {
    ...
    if ( item.meta.tjsLevel ) return "Level " + Number( item.meta.tjsLevel ) + " item.\\LB";
    if ( item.meta.tjsItemLevel ) return "Level " + Number( item.meta.tjsItemLevel ) + " item.\\LB";
    if ( item.meta.tjsWeaponLevel ) return "Level " + Number( item.meta.tjsWeaponLevel ) + " item.\\LB";
...
TJS.prototype.GetTextInfoTraits = function(traits, col_width_chars=64, cols=1) {
    ...
    for ( let i = 0; i < traits_size; i++ ) {
        ...
        /*
        if ( (trait.code == Game_BattlerBase.TRAIT_STATE_RATE) || (trait.code == Game_BattlerBase.TRAIT_STATE_RESIST) ) {
            if ( $dataStates[trait.dataId].multi_level_state > 0 ) {
                const state_single_id = $dataStates[trait.dataId].multi_level_state;
                const state_multi_start_id = state_single_id > 0 ? $dataStates[state_single_id].multi_level_state_averaged : -1;

                if ( trait.dataId != state_multi_start_id ) txt = "";
            }
        }
        */
        ...
        // if ( (trait.code == Game_BattlerBase.TRAIT_STATE_RATE) || (trait.code == Game_BattlerBase.TRAIT_STATE_RESIST) ) {
            ...
            */
            ...
            if ( $dataStates[trait.dataId].meta ) {
                if ( $dataStates[trait.dataId].meta.tjsMultiLevelState ) {
                    const state_single_id = Number( $dataStates[trait.dataId].meta.tjsMultiLevelState );
                    const state_multi_start_id = Number( $dataStates[state_single_id].meta.tjsMultiLevelStateAveraged )
...
TJS.prototype.IsStateIdSingle = function(state_id) {
    /*
    if ( ($dataStates[state_id].multi_level_state > 0) || ($dataStates[state_id].multi_level_state_averaged > 0) )
        return false;
    */
...
TJS.prototype.GetSingleStateAndDropIfPastFirst = function(state_id) {
    /*
    if ( $dataStates[state_id].multi_level_state > 0 ) {
        const id_single = $dataStates[state_id].multi_level_state;
        const id_starting = $dataStates[id_single].multi_level_state_averaged;

        if ( state_id == id_starting )
            return state_id;
        else
            return -1;
    }
    else if ( $dataStates[state_id].multi_level_state_averaged > 0 ) {
        return -1;
    }
    */
    ...
    /*
    if ( $dataStates[state_id].meta ) {
        if ( $dataStates[state_id].meta.tjsMultiLevelState ) {
            const id_single = Number( $dataStates[state_id].meta.tjsMultiLevelState );
            const id_starting = Number( $dataStates[id_single].meta.tjsMultiLevelStateAveraged );
        ...
        else if ( $dataStates[state_id].meta.tjsMultiLevelStateAveraged ) {
            return -1;
        }
...
TJS.prototype.GetStateAndResistance = function(actor, state_id) {
    ...
    /*
    if ( $dataStates[state_id].multi_level_state_averaged > 0 ) {
        const id_starting = $dataStates[state_id].multi_level_state_averaged;
        var accum = 0;

        for ( let i = 0; i < 10; i++ ) {
            accum += actor.stateRate( i + id_starting );
        }
        accum *= 0.10000;

        const resistance = Math.round((1.00000 - accum) * 10000.00000) / 100;
    ...
    else if ( $dataStates[state_id].multi_level_state > 0 ) {
        const id_single = $dataStates[state_id].multi_level_state;
        const id_starting = $dataStates[id_single].multi_level_state_averaged;
        var accum = 0;
...
TJS.prototype.GetStateImmunityId = function(actor, state_id) {
    /*
    if ( $dataStates[state_id].multi_level_state > 0 )
        return $dataStates[state_id].multi_level_state;
    */
...
TJS.prototype.FilterItem_Has_Meta_WeaponType = function(item) {
    ...
    if ( !item.meta.tjsWeaponType ) return false;
    return true;
    */
};
TJS.prototype.FilterItem_Has_Meta_ArmorType_EquipType = function(item) {
    ...
    /*
    if ( !item.meta ) return false;
    if ( !item.meta.tjsArmorType ) return false;
    if ( !item.meta.tjsEquipType ) return false;
    return true;
    */
};
TJS.prototype.FilterItem_Has_Meta_SlotId = function(item, slot_id) {
    if ( item.equip_type == slot_id ) return true;
    // if ( ((item.weapon_type > 0) || (item.armor_type > 0)) && (item.equip_type == slot_id) ) return true;
    return false;

    /*
    if ( !item.meta ) return false;
    if ( (!item.meta.tjsWeaponType) && (!item.meta.tjsArmorType) ) return false;
    if ( item.etypeId ) {
        if ( item.etypeId == slot_id ) return true;
    }
    return false;
    */
};
TJS.prototype.FilterItem_Has_Meta_Priority = function(item) {
    ...
    /*
    if ( !item.meta ) return false;
    if ( !item.meta.tjsPriority ) return false;
    if ( item.itypeId != 1 ) return false;
    return true;
    */
};
...
TJS.prototype.GetDiscoveredEnemies = function() {
    ...
    for ( let i = 2; i <= TJS_auto.highest_enemy_category; i++ ) {
        const enemies_specific_level = discovered_enemies.filter( enemy => i == enemy.category_index[0] );

        if ( enemies_specific_level.length > 0 ) enemies_listed = this.DeepCopy(enemies_listed.concat( enemies_specific_level ));
    }

    return enemies_listed;
    */
};
...
TJS.prototype.CalcExpValue = function( level_factor ) {
    ...
    const pow_min = 0.0;
    const pow_max = 14.0;
    const pow_skew = 0.0; // 0.3250;

    const xp_base_ref_1 = Math.pow( pow_base, TJS_auto.LerpSkew( pow_min, pow_max, 1.00000 / tjs_max_character_level, pow_skew ) );
    // const xp_base_ref_5 = Math.pow( pow_base, TJS_auto.LerpSkew( pow_min, pow_max, 5.00012 / tjs_max_character_level, pow_skew ) );
    // const xp_base_ref = 100.00000 / (xp_base_ref_5 - xp_base_ref_1);
    const xp_base_ref_max = Math.pow( pow_base, pow_max );
    const xp_base_ref = 3.82500 * Math.pow( 10, 14 ) / (xp_base_ref_max - xp_base_ref_1); // 250,000,000,000,000

    const xp_base = Math.pow( pow_base, TJS_auto.LerpSkew( pow_min, pow_max, level_factor.clamp( 0.0, 1.0 ), pow_skew ) );
    const xp_result = (xp_base - xp_base_ref_1) * xp_base_ref;
...
Game_Player.prototype.DoesEnemyHaveCategory = function(enemy, category) {
    ...
    /*
    if ( (enemy.category_index[0] == category) ||
        (enemy.category_index[1] == category) ||
        (enemy.category_index[2] == category) ||
        (enemy.category_index[3] == category) ||
        (enemy.category_index[4] == category) ||
        (enemy.category_index[5] == category) ||
        (enemy.category_index[6] == category) ||
        (enemy.category_index[7] == category) ||
        (enemy.category_index[8] == category) ||
        (enemy.category_index[9] == category) )
            return true;
    */
...
Game_Player.prototype.GetPartyLevel = function(level_var) {
    $gameVariables.setValue( level_var, $gameSystem.average_actor_level );
};
Game_Player.prototype.ShopWeapons = function(level_var, buy_only=false) {
    const level = $gameVariables.value( level_var ).clamp( 1, tjs_max_item_levels );
    var items = [];

    const weapons = $dataWeapons.filter( item => item.level == level );
    const weapons_normal = weapons.filter( item => item.special == TJS_ITEM_SPECIAL_NORMAL );
...
Game_Player.prototype.ShopArmors = function(level_var, buy_only=false) {
    ...
    const armors = $dataArmors.filter( item => item.level == level );
    const armors_normal = armors.filter( item => item.special == TJS_ITEM_SPECIAL_NORMAL );

    for ( const armor of armors_normal ) {
        const item = [ 1, armor.id, 0 ];
        items.push( item );
    }
...
Window_EquipItem.prototype.includes = function(item_input) {
    ...
    // const item = (item_input.wtypeId) ? $dataWeapons[item_input.id] : $dataArmors[item_input.id];
    var item = {};
...
Window_SkillList.prototype.makeItemList = function() {
    ...
    // const all_items = JSON.parse(JSON.stringify( this._actor.skills().filter(item => this.includes(item)) ));
    ...
    /*
    const all_items_length = all_items.length;

    const items_has_meta = JSON.parse(JSON.stringify( all_items.filter( item => (item.meta) ) ));
    const items_no_meta = JSON.parse(JSON.stringify( all_items.filter( item => (!item.meta) ) ));

    const items_has_priority = JSON.parse(JSON.stringify( items_has_meta.filter( item => (item.meta.tjsPriority) ) ));
    const items_no_priority = JSON.parse(JSON.stringify( items_has_meta.filter( item => (!item.meta.tjsPriority) ) ));

    for ( let i = 1; i <= tjs_highest_item_priority; i++ ) {
        const items_has_specific_priority = JSON.parse(JSON.stringify( items_has_priority.filter( item => (item.meta.tjsPriority == i) ) ));
        if ( items_has_priority.length > 0 ) this._data = JSON.parse(JSON.stringify( this._data.concat(items_has_specific_priority) ));
    }

    if ( items_no_priority.length > 0 ) this._data = JSON.parse(JSON.stringify( this._data.concat(items_no_priority) ));
    if ( items_no_meta.length > 0 ) this._data = JSON.parse(JSON.stringify( this._data.concat(items_no_meta) ));
    */
};
...
Game_Party.prototype.maxItems = function(/*item*/item) {
    ...
    /*
    // check for weapons vs armors vs items
    // if ( item.etypeId ) return count;
    if ( item.wtypeId ) {
        if ( $dataWeapons[item.id].meta ) {
            if ( $dataWeapons[item.id].meta.tjsMaxCount ) count = Number($dataWeapons[item.id].meta.tjsMaxCount);
        }
    ...
    if ( item.atypeId ) {
        if ( $dataArmors[item.id].meta ) {
            if ( $dataArmors[item.id].meta.tjsMaxCount ) count = Number($dataArmors[item.id].meta.tjsMaxCount);
        }
...
Game_Actor.prototype.setup = function(actorId) {
    ...
    /*
    this._breakingCountWeapon1 = 0;
    this._breakingCountWeapon2 = 0;
    this._breakingCountWeapon3 = 0;
    */
    this._equipsLevels = [];
    // this._tacticalPointsMod = 1;
    // this.tp_max = 100;
...
Game_Actor.prototype.expForLevel = function(level) {
    ...
    // const xp_base = Math.pow( 10, TJS_auto.Lerp( 1.00000, 11, level_fac ) ) * 5.6234 * above_zero; // 5.56
...
Game_Actor.prototype.GetLevelMultiplierForEquip = function(slot_index) {
    ...
    if ( slot_index <= 1 ) {
        if ( this._equips[slot_index] != null ) {
            if ( $dataWeapons[this._equips[slot_index]._itemId].meta ) {
                if ( $dataWeapons[this._equips[slot_index]._itemId].meta.tjsItemLevel ) levelFactor = Number($dataWeapons[this._equips[slot_index]._itemId].meta.tjsItemLevel);
                if ( $dataWeapons[this._equips[slot_index]._itemId].meta.tjsWeaponLevel ) levelFactor = Number($dataWeapons[this._equips[slot_index]._itemId].meta.tjsWeaponLevel);
                if ( $dataWeapons[this._equips[slot_index]._itemId].meta.tjsLevel ) levelFactor = Number($dataWeapons[this._equips[slot_index]._itemId].meta.tjsLevel);
            }
        }
    }
...
Game_Actor.prototype.GetSlotOfSkill = function(skill) {
    ...
    if ( !$dataSkills[skill.id].meta ) return skill_slot;

    const skill_meta = $dataSkills[skill.id].meta;
    if ( skill_meta.tjsWeapon ) {
        const weapon_slot = TJS_auto.GetEquipSlotOfSkill( skill, this );
        if ( weapon_slot >= 0 ) skill_slot = weapon_slot;
    }
    else if ( skill_meta.tjsMinor ) skill_slot = TJS_auto.tjsMinor - 1;
    else if ( skill_meta.tjsSpell ) skill_slot = TJS_auto.tjsSpell - 1;
    else if ( skill_meta.tjsBook ) skill_slot = TJS_auto.tjsBook - 1;
    else if ( skill_meta.tjsShield ) skill_slot = TJS_auto.tjsShield - 1;
    else if ( skill_meta.tjsRing ) skill_slot = TJS_auto.tjsRing - 1;
    else if ( skill_meta.tjsAccessory ) skill_slot = TJS_auto.tjsAccessory - 1;
...
Game_Actor.prototype.GetLevelOfEquippedItemOfSlot = function(slot_index) {
    ...
    /*
    if ( slot_index <= 1 ) {
        if ( this._equips[slot_index] ) {
            if ( !this._equips[slot_index]._itemId ) return levelFactor;
...
Game_Actor.prototype.GetLevelOfSkill = function(skill) {
    ...
    // if ( skill_slot < this._equipsLevels.length ) skill_level = this._equipsLevels[skill_slot];

    return skill_level;
};
/*
Game_Actor.prototype.GetTpBase = function() {
    return this.tp_max + $dataClasses[ this.currentClass().id ].tp_max_modifier;
};
Game_Actor.prototype.CalculateTpModifier = function() {
    this._tacticalPointsMod = 1.00000;
    const equips_size = this.equips().length;

    for (let i = 0; i < equips_size; i++) {
        if ( !this.equips()[i] ) continue;
        this._tacticalPointsMod += this.equips()[i].tp_modifier - 1.00000;
    }
};
*/
...
Game_Actor.prototype.GetLevelValueForSkill = function(skill_id) {
    ...
    /*
    if (slot == 0) {
        if ( !this.equips()[0] ) return 0.00000;

        var lvl = 1;

        if ( $dataWeapons[this.equips()[0].id].meta ) {
            if ( $dataWeapons[this.equips()[0].id].meta.tjsWeaponLevel ) lvl = Number( $dataWeapons[this.equips()[0].id].meta.tjsWeaponLevel );
            if ( $dataWeapons[this.equips()[0].id].meta.tjsBookLevel ) lvl = Number( $dataWeapons[this.equips()[0].id].meta.tjsBookLevel );
            if ( $dataWeapons[this.equips()[0].id].meta.tjsItemLevel ) lvl = Number( $dataWeapons[this.equips()[0].id].meta.tjsItemLevel );
            if ( $dataWeapons[this.equips()[0].id].meta.tjsLevel ) lvl = Number( $dataWeapons[this.equips()[0].id].meta.tjsLevel );
        }

        lvl -= 1;
        return lvl / (tjs_max_item_levels - 1.00000);
    }
    else if (slot == 1) {
        if ( !this.equips()[1] ) return 0.00000;

        var lvl = 1;

        if ( $dataWeapons[this.equips()[1].id].meta ) {
            if ( $dataWeapons[this.equips()[1].id].meta.tjsWeaponLevel ) lvl = Number( $dataWeapons[this.equips()[1].id].meta.tjsWeaponLevel );
            if ( $dataWeapons[this.equips()[1].id].meta.tjsBookLevel ) lvl = Number( $dataWeapons[this.equips()[1].id].meta.tjsBookLevel );
            if ( $dataWeapons[this.equips()[1].id].meta.tjsItemLevel ) lvl = Number( $dataWeapons[this.equips()[1].id].meta.tjsItemLevel );
            if ( $dataWeapons[this.equips()[1].id].meta.tjsLevel ) lvl = Number( $dataWeapons[this.equips()[1].id].meta.tjsLevel );
        }
        ...
        if ( !this.equips()[slot] ) return 0.00000;

        var lvl = 1;

        if ( $dataArmors[this.equips()[slot].id].meta ) {
            if ( $dataArmors[this.equips()[slot].id].meta.tjsWeaponLevel ) lvl = Number( $dataArmors[this.equips()[slot].id].meta.tjsWeaponLevel );
            if ( $dataArmors[this.equips()[slot].id].meta.tjsBookLevel ) lvl = Number( $dataArmors[this.equips()[slot].id].meta.tjsBookLevel );
            if ( $dataArmors[this.equips()[slot].id].meta.tjsItemLevel ) lvl = Number( $dataArmors[this.equips()[slot].id].meta.tjsItemLevel );
            if ( $dataArmors[this.equips()[slot].id].meta.tjsLevel ) lvl = Number( $dataArmors[this.equips()[slot].id].meta.tjsLevel );
        }
...
Game_Actor.prototype.profile = function() {
    ...
    if ( $dataActors[this.actorId()].meta ) {
        if ( $dataActors[this.actorId()].meta.tjsInfo ) {
                var profile_text = $dataActors[this.actorId()].meta.tjsInfo;

                return TJS_auto.ReparseTextAuto( profile_text, tjs_string_line_width_noFace );
        }
    }
    */
    ...
    for ( let i = 0; i < all_trait_objects.length; i++ ) {
        ...
        for ( let t = 0; t < traits_size; t++ ) {
            ...
            if ( trt.code == Game_BattlerBase.TRAIT_STATE_RESIST ) {
                ...
                /*
                if ( $dataStates[state_id].multi_level_state > 0 ) {
                    state_id = TJS_auto.GetStateImmunityId( this, state_id );
                }
                */
...
Game_Actor.prototype.TraitsEffectsPartiallyResistantTo = function() {
    ...
    // const all_traits = all_trait_objects.map( trait_object => trait_object.traits ).flat();
...
Game_Actor.prototype.StatesPartiallyResistant = function() {
    ...
    const traits_resistances = TJS_auto.DeepCopy( all_trait_objects.filter( trait => trait.code === Game_BattlerBase.TRAIT_STATE_RATE ) );
    // const traits_resistances = TJS_auto.DeepCopy( all_trait_objects ).sort( (a, b) => (a.value - b.value) );
    const traits_immunities_ids = all_trait_objects.filter( trait => trait.code === Game_BattlerBase.TRAIT_STATE_RESIST ).map( trait => trait.dataId );
    ...
    /*
    const all_traits = all_trait_objects.map( trait_object => trait_object.traits ).flat();
    const traits_resistances = TJS_auto.DeepCopy(all_traits.filter( trait => trait.code == Game_BattlerBase.TRAIT_STATE_RATE )).sort( (a, b) => (b.value - a.value) ).map( trait => trait.dataId );
    const traits_immunities = all_traits.filter( trait => trait.code == Game_BattlerBase.TRAIT_STATE_RESIST ).map( trait => trait.dataId );

    for ( const trait of traits_resistances ) {
        if ( traits_immunities.includes(trait) ) continue;
        if ( states_partially_resistant.includes(trait) ) continue;

        states_partially_resistant.push( trait );
    }
    */
    ...
    for ( let i = 0; i < all_trait_objects.length; i++ ) {
        ...
        for ( let t = 0; t < traits_size; t++ ) {
            ...
            if ( trt.code == Game_BattlerBase.TRAIT_STATE_RATE ) {
                var state_id = trt.dataId;
                if ( !$dataStates[state_id] ) continue;
                // if ( states_immune.some( state => state.dataId == trt.dataId ) ) continue;

                if ( $dataStates[state_id].multi_level_state > 0 ) {
                    const state_data = TJS_auto.GetStateAndResistance( this, state_id );
                    if ( state_data ) state_id = state_data[0];
                }

                if ( !states_partially_resistant.includes(state_id) ) states_partially_resistant.push( state_id );
            }
        }
    }
    */
...
Game_Enemy.prototype.TjsSetupCustom = function() {
    ...
    this.CalculateLevel();
    // this.profile = enemy.profile;
    // this.tp_max = enemy.tp_max;
    this.initial_tp = enemy.initial_tp;
    // this.to_hit_physical = enemy.to_hit_physical;
    // this.to_hit_magical = enemy.to_hit_magical;
...
Game_Enemy.prototype.GetAtr_MagicFocus = function() {
    ...
    // value = (value * this.level_factor * 20.0).clamp( 1, 500000 );
...
Game_Enemy.prototype.GetUsableFreeSkills = function() {
    ...
    // const usable_skills = (this.enemy().actions.map( action => $dataSkills[action.skillId] )).filter(skill => this.canUse(skill));
...
Game_Enemy.prototype.selectAllActions = function(actionList) {
    ...
    // actionList = actionList.filter( (a, idx) => (a.rating > 0) && (this.IsActionIndexAllowedBySkillRule( idx )) );
...
Game_Enemy.prototype.GetExpForLevel = function(level) {
    ...
    const above_zero = level > 1 ? 1 : 0;
    // const xp_base = Math.pow( 10, TJS_auto.Lerp( 1.00000, 11, level_fac ) ) * 5.56 * above_zero;
    const xp_base = TJS_auto.CalcExpValue( level_fac ) * above_zero;
...
Game_Enemy.prototype.exp = function() {
    ...
    // const exp_diff = this.GetExpForLevel( (this.level + 1).clamp(2, tjs_max_monster_level) ) - this.GetExpForLevel( this.level.clamp(1, tjs_max_monster_level - 1) );
...
Game_Enemy.prototype.gold = function() {
    ...
    return ( (this.enemy().gold * 0.01000 * exp_diff) * tjs_enemy_gold_multiplier * this.level_factor );
    */
...
Window_ShopSell.prototype.isEnabled = function(item) {
    ...
    if ( item.meta ) {
        if ( (item.meta.tjsLeveledByChapterPrice) || (item.meta.tjsWeapon) || (item.meta.tjsGear) || (item.meta.tjsArmor) || (item.meta.tjsBook) ) return item && TJS_auto.CalcLevelByChapterPrice( item.price, item_level ) > 0;
    }
    */
...
Scene_Shop.prototype.sellingPrice = function() {
    ...
    if ( this._item.meta ) {
        ...
        if ( item.meta.tjsFoodPrice ) return Math.ceil( TJS_auto.CalcFoodPrice(Number(item.meta.tjsFoodPrice)) / tjs_item_selling_divisor );

        if ( item.meta.tjsItemLevel ) item_level = Number(item.meta.tjsItemLevel);
        if ( item.meta.tjsWeaponLevel ) item_level = Number(item.meta.tjsWeaponLevel);
        if ( item.meta.tjsLevel ) item_level = Number(item.meta.tjsLevel);

        if ( item_level < 1 ) return Math.ceil( sell_price / tjs_item_selling_divisor );

        if ( (item.meta.tjsLeveledByChapterPrice) || (item.meta.tjsWeapon) || (item.meta.tjsGear) || (item.meta.tjsArmor) || (item.meta.tjsBook) )
            sell_price = TJS_auto.CalcLevelByChapterPrice( sell_price, item_level );

        return Math.ceil( sell_price / tjs_item_selling_divisor );
    }

    return Math.ceil( this._item.price / tjs_item_selling_divisor );
    */
};
...
Game_Action.prototype.itemEffectAddAttackState = function(target, effect) {
    console.log( this.subject().name() + " vs " + target.name() + ", skill: " + this.item().name );
    console.log( this.subject().attackStates() );
    console.log( this.item().effects );

    for (const stateId of this.subject().attackStates()) {
        let chance = effect.value1;

        console.log( "this.subject().attackStatesRate(stateId): " + this.subject().attackStatesRate(stateId) + ", " + $dataStates[stateId].name );
        console.log( "chance: " + chance );
        console.log( "target.stateRate(stateId): " + target.stateRate(stateId) );
...
/*
Window_Base.prototype.CustomHpColor = function( value, value_max ) {
    const percent = 100.0 * value / value_max;

    if ( value <= 0 ) this.changeTextColor( tjs_color_hp_zero );
    else if ( percent <= 8 ) this.changeTextColor( tjs_color_hp_near_zero );
    else if ( percent <= 15 ) this.changeTextColor( tjs_color_hp_very_low );
    else if ( percent <= 25 ) this.changeTextColor( tjs_color_hp_low );
    else if ( percent <= 32 ) this.changeTextColor( tjs_color_hp_medium_low );
    else if ( percent <= 64 ) this.changeTextColor( tjs_color_hp_medium );
    else if ( value < value_max ) this.changeTextColor( tjs_color_hp_slight );
    else this.changeTextColor( tjs_color_hp_max );
};
*/
...
Scene_Base.prototype.GetEquipStatsWindowWidth = function() { // GetEquipWindowWidth
    // 1920 pixels - ( padding_of_both_sides + equipment_slot_name_width + item_icon_plus_icon_padding + width_of_item_name ) = 1437.
    // 1920 - ( (5 * 2) + (9.5 * 16) + 36 + (9.5 * 36) ) = 1399.
    // 1920 - 521 = 1380.
    return 1380; // 1388
};
...
Window_MenuStatus.prototype.DrawActiveStates = function(index) {
    ...
    const x = rect.x + 530;
    // const x = rect.x;
    // const x_width = this.textWidth("Effects of damage types");
    const y = rect.y; // + Math.floor(rect.height / 2 - this.lineHeight() * 1.5);
...
Sprite_Gauge.prototype.drawValue = function() {
    ...
    /*
    var slash = " / ";
    if ( (this.currentValue() > 999999.75) || (this.currentMaxValue() > 999999.75) )
        slash = "/";
    */
...
Window_EquipStatus.prototype.drawAllParams = function() {
    ...
    */
    ...
    // this.drawItemX(this.itemPadding(), this.paramY(13), 2);
    ...
    // this.drawItemX(this.itemPadding(), this.paramY(17), 3);
    ...
    /*
    if ( this._tempActor ) {
        this.drawRightArrow( this.CharWidth() * 74, this.lineHeightB() * 1 ); // 720, this.lineHeightB() * 1
        this.drawRightArrow( this.CharWidth() * 74, this.lineHeightB() * 19 ); // 720, this.lineHeightB() * 19
        this.DrawListOfStatesImmuneTo( this._tempActor, 750, this.lineHeightB() * 1, 16 ); // this._tempActor, 750, this.lineHeightB() * 1, 16
        this.DrawListOfStatesResistantToCompare( this._actor, 750, this.lineHeightB() * 19, 19, this._tempActor ); // this._actor, 750, this.lineHeightB() * 19, 23, this._tempActor
    }
    */
};
...
Window_StatusBase.prototype.DrawListOfStatesResistantToTextSingle = function(actor, x, y, maxLineCount) {
    ...
    for ( let i = 0; i < states_resistant_size; i++ ) {
        ...
        // this.drawText( (actor.GetStatePartialResistancePercent( states_resistant[i] )).toLocaleString() + "%", x, y + (lineHeight * i), entry_width, "right" );
...
Window_StatusBase.prototype.Draw_Params = function(x, y) {
    ...
    /*
    for (let i = 2; i < 9; i++) {
        this.Draw_Item(x, y + (i * height) + height, i);
    }
    */
};
...
Window_StatusBase.prototype.Draw_ParamsS = function(x, y) {
    ...
    /*
    this.Draw_ItemS(x, y, 4);
    this.Draw_ItemS(x, y + 22, 5);
    this.Draw_ItemS(x, y + 44, 6);
    this.Draw_ItemS(x, y + 66, 7);
    this.Draw_ItemS(x, y + 88, 9);
    this.Draw_ItemS(x, y + 110, 0);
    */
...
Window_StatusBase.prototype.Draw_CurrentParamX = function(x, y, xparamId) {
    ...
    // const end_char = xparamId == 1 || xparamId == 4 ? " " : "%";
    this.resetTextColor();
    this.drawText(v, x, y, paramWidth, "right");
};
...
Window_Status.prototype.refresh = function() {
    ...
    if (this._actor) {
        ...
        // this.lineHeightB() * 38 is last line
    }
};
...
Scene_Equip.prototype.statusWidth = function() {
    ...
    // 1920 pixels - ( padding_of_both_sides + item_icon_plus_icon_padding + width_of_item_name ) = 1437.
    // return 1437;

    // Old.
    // return 1413; // 312, 1350, 1100, 1125
};
...
Scene_Status.prototype.statusWindowRect = function() {
    ...
    const ww = Graphics.boxWidth;
    // const ww = this.TjsGetStatsBoxWidth(); // Graphics.boxWidth;
...
Scene_Status.prototype.TjsGetStatsBoxWidth = function() {
    ...
    // return TJS_auto.Avg( this.GetEquipStatsWindowWidth(), Graphics.boxWidth );

    // Old.
    // return 1345; // 1110, 1350
};
...
Window_Help.prototype.setItem = function(item) {
    // this.setText(item ? TJS_auto.ReparseTextAuto(item.description) : "");
...
/*
    this.resetTextColor();
    this.changeTextColor( ColorManager.hpColor(this._battler) );
    this.changeTextColor( tjs_color_tp );
    this.changeTextColor( tjs_color_mp );
    this.changeTextColor( tjs_color_lvl );
*/
...
DataManager.PrepareTjsData_Special = function(item) {
    if ( !item ) return TJS_ITEM_SPECIAL_NORMAL;
    if ( !item.special ) item.special = TJS_ITEM_SPECIAL_NORMAL;
    if ( item.special != TJS_ITEM_SPECIAL_NORMAL ) return item.special;
    ...
    var special = TJS_ITEM_SPECIAL_NORMAL;

    // Shared Items
    if ( item.itypeId ) {
        if ( item.itypeId == 1 ) {
            special = TJS_ITEM_SPECIAL_MONSTER_DROP;

            if ( txt.includes("healing potion") ) special = TJS_ITEM_SPECIAL_HEALING_HP;
            else if ( txt.includes("magic syrup") ) special = TJS_ITEM_SPECIAL_HEALING_MP;
            else if ( txt.includes("tactical endurance potion") ) special = TJS_ITEM_SPECIAL_HEALING_TP;
            else if ( txt.includes("hyperlife injection") ) special = TJS_ITEM_SPECIAL_HEALING_DEATH;
            else if ( item.description.includes("Permanantly increases ") ) special = TJS_ITEM_SPECIAL_ABILITY_GROW;
            else if ( item.description.includes("Buffs one's ") ) special = TJS_ITEM_SPECIAL_ABILITY_BOOST;
            else if ( txt.includes("magic flower") ) special = TJS_ITEM_SPECIAL_POWER_UP_GENERIC;
        }
            ...
            if ( txt.includes("orb") ) special = TJS_ITEM_SPECIAL_ORB;
            if ( txt.includes("orb shards") ) special = TJS_ITEM_SPECIAL_ORB_SHARDS;
        }
    ...
    // Broken Items
    if ( (weapon_type == 1) || (armor_type == 1) ) {
        special = TJS_ITEM_SPECIAL_BROKEN;

        return special;
    }
    // Weapons and Backup Weapons
    else if ( (equip_type == 1) || (equip_type == 3) ) {
        if ( txt.includes("fire") ) special = TJS_ITEM_SPECIAL_FIRE;
        else if ( txt.includes("flaming") ) special = TJS_ITEM_SPECIAL_FIRE;
        else if ( txt.includes("incendiary") ) special = TJS_ITEM_SPECIAL_FIRE;
        else if ( txt.includes("ice") ) special = TJS_ITEM_SPECIAL_ICE;
        else if ( txt.includes("freezing") ) special = TJS_ITEM_SPECIAL_ICE;
        else if ( txt.includes("lightning") ) special = TJS_ITEM_SPECIAL_LIGHTNING;
        else if ( txt.includes("thunder") ) special = TJS_ITEM_SPECIAL_LIGHTNING;
        else if ( txt.includes("electric") ) special = TJS_ITEM_SPECIAL_LIGHTNING;
        else if ( txt.includes("shocking") ) special = TJS_ITEM_SPECIAL_LIGHTNING;
        else if ( txt.includes("metal") ) special = TJS_ITEM_SPECIAL_METAL;
        else if ( txt.includes("acid") ) special = TJS_ITEM_SPECIAL_ACID;
        else if ( txt.includes("water") ) special = TJS_ITEM_SPECIAL_WATER;
        else if ( txt.includes("earth") ) special = TJS_ITEM_SPECIAL_EARTH;
        else if ( txt.includes("wind") ) special = TJS_ITEM_SPECIAL_WIND;
        else if ( txt.includes("hasted") ) special = TJS_ITEM_SPECIAL_HASTED;
        else if ( txt.includes("bleeding") ) special = TJS_ITEM_SPECIAL_BLEEDING;
        else if ( txt.includes("blood") ) special = TJS_ITEM_SPECIAL_BLEEDING;
        else if ( txt.includes("stunning") ) special = TJS_ITEM_SPECIAL_STUNNING;
        else if ( txt.includes("stun") ) special = TJS_ITEM_SPECIAL_STUNNING;
        else if ( txt.includes("antimagical") ) special = TJS_ITEM_SPECIAL_ANTIMAGICAL;
        else if ( txt.includes("anti-magical") ) special = TJS_ITEM_SPECIAL_ANTIMAGICAL;
        else if ( txt.includes("antimagic") ) special = TJS_ITEM_SPECIAL_ANTIMAGICAL;
        else if ( txt.includes("anti-magic") ) special = TJS_ITEM_SPECIAL_ANTIMAGICAL;
        else if ( txt.includes("draining") ) special = TJS_ITEM_SPECIAL_ANTIMAGICAL;
        else if ( txt.includes("power") ) special = TJS_ITEM_SPECIAL_POWER;
        else if ( txt.includes("mega") ) special = TJS_ITEM_SPECIAL_POWER;
        else if ( txt.includes("rune") ) special = TJS_ITEM_SPECIAL_WEAPON_RUNE;
    }
    // Character Class Books
    else if ( equip_type == 4 ) {
        special = TJS_ITEM_SPECIAL_CLASS_BOOK;
    }
    // Shields
        ...
        else if ( txt.includes("darkness") ) special = TJS_ITEM_SPECIAL_DARKNESS;
        else if ( txt.includes("immortality") ) special = TJS_ITEM_SPECIAL_IMMORTALITY;
        else if ( txt.includes("rainbow") ) special = TJS_ITEM_SPECIAL_RAINBOW;
        else if ( txt.includes("rune") ) special = TJS_ITEM_SPECIAL_ARMOR_RUNE;
    }
    // Armored Clothing
    else if ( (equip_type >= 6) || (equip_type <= 11) ) {
        special = TJS_ITEM_SPECIAL_NORMAL;

        if ( txt.includes("rune") ) special = TJS_ITEM_SPECIAL_ARMOR_RUNE;
    }
    // Magical Clothing and accessories
    else if ( (equip_type >= 12) || (equip_type <= 13) ) {
        special = TJS_ITEM_SPECIAL_ENCHANTED;

        if ( txt.includes("rune") ) special = TJS_ITEM_SPECIAL_ARMOR_RUNE;
    }
    // Rings
    else if ( equip_type == 14 ) {
        special = TJS_ITEM_SPECIAL_GEAR_MAGICAL_RING_LEVELED;

        if ( item.has_levels == false ) {
            if ( txt.includes("minor") ) special = TJS_ITEM_SPECIAL_GEAR_MAGICAL_RING_MINOR;
            else if ( txt.includes("major") ) special = TJS_ITEM_SPECIAL_GEAR_MAGICAL_RING_MAJOR;
            else if ( txt.includes("ultra") ) special = TJS_ITEM_SPECIAL_GEAR_MAGICAL_RING_ULTRA;
        }
    }
    // Accessories
        ...
        if ( txt.includes("rune") ) special = TJS_ITEM_SPECIAL_RUNE;
    }
...
DataManager.CreateTjsDataFile_Weapons = function() {
    ...
    for ( i = 21; i < $dataWeapons.length; i++ ) {
        ...
        if ( item.special == TJS_ITEM_SPECIAL_BROKEN ) continue;

        if ( item.description2 ) delete item.description2;

        data_unleveled.push( TJS_auto.DeepCopy(item) );
    }
    ...
    for ( i = 21; i < $dataWeapons.length; i++ ) {
        ...
        data_leveled.push( TJS_auto.DeepCopy(item) );
    }
...
DataManager.CreateTjsDataFile_Armors = function() {
    ...
    for ( i = 21; i < $dataArmors.length; i++ ) {
        ...
        if ( item.id <= 20 ) continue;
        if ( item.special == TJS_ITEM_SPECIAL_BROKEN ) continue;
        if ( item.special == TJS_ITEM_SPECIAL_GEAR_MAGICAL_RING_MINOR ) continue;
        if ( item.special == TJS_ITEM_SPECIAL_GEAR_MAGICAL_RING_MAJOR ) continue;
        if ( item.special == TJS_ITEM_SPECIAL_GEAR_MAGICAL_RING_ULTRA ) continue;
...
DataManager.WriteOldFormatDataFile_Weapons = function(data_array) {
    for ( var data of data_array ) {
        ...
        if ( data.treasure_type == TJS_ITEM_TREASURE_TYPE_PRIMARY_NONE ) data.note += "<tjsTreasurePrimaryNone>\n";
        else if ( data.treasure_type == TJS_ITEM_TREASURE_TYPE_PRIMARY_COMMON ) data.note += "<tjsTreasurePrimaryCommon>\n";
        else if ( data.treasure_type == TJS_ITEM_TREASURE_TYPE_PRIMARY_UNCOMMON ) data.note += "<tjsTreasurePrimaryUncommon>\n";
        else if ( data.treasure_type == TJS_ITEM_TREASURE_TYPE_PRIMARY_RARE ) data.note += "<tjsTreasurePrimaryRare>\n";
        else if ( data.treasure_type == TJS_ITEM_TREASURE_TYPE_SECONDARY_COMMON ) data.note += "<tjsTreasureSecondaryCommon>\n";
        else if ( data.treasure_type == TJS_ITEM_TREASURE_TYPE_SECONDARY_UNCOMMON ) data.note += "<tjsTreasureSecondaryUncommon>\n";
        else if ( data.treasure_type == TJS_ITEM_TREASURE_TYPE_SECONDARY_RARE ) data.note += "<tjsTreasureSecondaryRare>\n";
...
BattleManager.changeCurrentActor = function(forward) {
    ...
    if ( this._actor_window ) {
        ...
        // console.log( actor_index );
        // this._actor_window.forceSelect( actor_index );
    }
...
BattleManager.displayGold = function() {
    ...
    /*
    if (gold > 0) {
        $gameMessage.add( "\\." + TextManager.obtainGold.format(gold) + "\\LB" );
    }
    */
...
BattleManager.displayDropItems = function() {
    ...
    if (items.length > 0) {
        ...
        for (const item of items) {
            // $gameMessage.add( TextManager.obtainItem.format(item.name) + "\\LB" );
...
Window_BattleLog.prototype.AddLine = function(text) {
    ...
    // this._lines[this._line_index] = text;
};
...
Scene_Skill.prototype.showActorWindow = function() {
    /*
    const index_mod = (this._itemWindow.index()).mod( this._itemWindow.maxCols() );
    const window_on_right = (index_mod >= 2) ? false : true;

    if ( window_on_right == true ) {
...
Scene_Boot.prototype.CustomTjsReallocateAndCompactLevels = function() {
    ...
    for ( let i = 21; i < weapons_size; i++ ) {
        ...
        new_weapons.push( blank.Deep() );
    }
    new_weapons.push( blank_weapon.Deep() );
...
Scene_Boot.prototype.CustomTjsInitObjectVariables = function() {
    ...
    for ( let i = 1; i < enemies_size; i++ ) { // Blagg
        ...
        if ( data.meta.tjsAttackLevels ) data.attack_levels = JSON.parse( data.meta.tjsAttackLevels );
        ...
        if ( data.meta.tjsSkillRules ) {
            ...
            const rule_list = JSON.parse( data.meta.tjsSkillRules );
    ...
    for ( let i = 1; i < weapons_size; i++ ) {
        ...
        data.special = TJS_ITEM_SPECIAL_NORMAL;
        data.treasure_type = TJS_ITEM_TREASURE_TYPE_PRIMARY_NONE;
        data.custom_traits = {
            to_hit_physical_add: 0,
            to_hit_physical_multiply: 1,
            to_hit_magical_add: 0,
        ...
        if ( data.meta.tjsTreasurePrimaryNone ) data.treasure_type = TJS_ITEM_TREASURE_TYPE_PRIMARY_NONE;
        if ( data.meta.tjsTreasurePrimaryCommon ) data.treasure_type = TJS_ITEM_TREASURE_TYPE_PRIMARY_COMMON;
        if ( data.meta.tjsTreasurePrimaryUncommon ) data.treasure_type = TJS_ITEM_TREASURE_TYPE_PRIMARY_UNCOMMON;
        if ( data.meta.tjsTreasurePrimaryRare ) data.treasure_type = TJS_ITEM_TREASURE_TYPE_PRIMARY_RARE;
        if ( data.meta.tjsTreasureSecondaryCommon ) data.treasure_type = TJS_ITEM_TREASURE_TYPE_SECONDARY_COMMON;
        if ( data.meta.tjsTreasureSecondaryUncommon ) data.treasure_type = TJS_ITEM_TREASURE_TYPE_SECONDARY_UNCOMMON;
        if ( data.meta.tjsTreasureSecondaryRare ) data.treasure_type = TJS_ITEM_TREASURE_TYPE_SECONDARY_RARE;
...
Scene_Boot.prototype.CustomTjsSetDataSkills = function() {
    ...
    for ( let i = 1; i < skills_size; i++ ) {
        ...
        // data.aim_adjust = TJS_auto.GetAimAdjustForSkill( data_equip_type, data_weapon_type, data_armor_type );
...
Scene_Boot.prototype.CustomTjsSetLeveledRates = function() {
    ...
    for ( let i = 1; i < weapons_size; i++ ) {
        ...
        var item = $dataWeapons[i];
        // const item_custom_traits_copy = TJS_auto.DeepCopy( item.custom_traits );
